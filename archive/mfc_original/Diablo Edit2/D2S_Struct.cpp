...
CString CD2S_Struct::name() const {
	if (IsPtr31AndAbove(dwVersion)) {
		// 0x69版本: NamePTR名字可能不在起始位置，需跳过前导零字节
...
void CD2S_Struct::name(const CString& name) {
	...
	if (isV31) {
		// 0x69版本: 名字存储在NamePTR中，从起始位置写入
		::ZeroMemory(NamePTR, sizeof NamePTR);
		::CopyMemory(NamePTR, utf8, min(utf8.GetLength(), (int)sizeof NamePTR));
	} else {
		// 旧版本
